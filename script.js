...
function startGame(level) {
  ...
  let phrases = level === "easy" ? allPhrases.slice(0, 4) : allPhrases;
...
function flipCard(card, phrase) {
  ...
  if (selected.length === 2) {
    isLocked = true; // bloqueia cliques
    ...
    if (card1.innerText === card2.innerText && card1 !== card2) {
      ...
      matchSound.play();
      isLocked = false; // libera imediatamente após acerto
    } else {
      wrongSound.play();
      setTimeout(() => {
        card1.innerHTML = "";
        card2.innerHTML = "";
        isLocked = false; // libera após 4 segundos
      }, 4000);
    }
    selected = [];
  }
